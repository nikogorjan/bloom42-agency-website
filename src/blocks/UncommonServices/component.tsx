...
export const UncommonServices: React.FC<UncommonServicesProps> = ({ features }) => {
  ...
  // Decide ranges first, then call useTransform once
  const { inputRange, outputRange } = useMemo(() => {
    const input = [0.3, 0.4, 0.5, 0.6, 0.7, 0.8]
    const outputTablet = ['0%', '-25%', '-25%', '-50%', '-50%', '-75%']
    const outputDesktop = ['0%', '0%', '-25%', '-35%', '-50%', '-75%']
    return { inputRange: input, outputRange: isTablet ? outputTablet : outputDesktop }
  }, [isTablet])
        ...
        <div className="relative grid auto-cols-fr grid-cols-1 items-start gap-x-8 gap-y-12 md:grid-cols-[0.75fr_1fr] md:gap-y-16 lg:grid-cols-[max-content_1fr] lg:gap-x-20">
          ...
          <div className="grid auto-cols-fr grid-cols-1 gap-x-12 gap-y-12 md:gap-x-28 md:gap-y-28">
            {safeFeatures.map((feature, index) => (
              <FeatureCard key={index} feature={feature as Feature} />
            ))}
          </div>
        </div>
      </div>
...
const FeatureCard = ({ feature }: { feature: Feature }) => {
  ...
  return (
    <div className="flex flex-col items-start justify-center py-8 md:py-0 ">
      ...
      {heading && (
        <h2 className="font-figtree font-semibold text-white mb-5 text-5xl md:mb-6 md:text-7xl lg:text-8xl">
          {heading}
        </h2>
      )}
